...
class Session(cmd.Cmd):
    ...
    def completemodules(self, text):
        ...
        if self.__base == "":
            ...
            modules = [[m[len(self.__base):] for m in modules if m.startswith(self.__base + text)]]
    ...
    def completenamespaces(self, text):
        ...
        if self.__base == "":
            ...
            return [[m[len(self.__base):] for m in self.__namespaces() if m.startswith(self.__base + text)]]
